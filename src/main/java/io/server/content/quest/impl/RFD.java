...
import io.server.content.activity.impl.recipefordisaster.RecipeForDisaster;
...
import io.server.content.store.impl.RecipeForDisasterStore;
...
public class RFD extends Quest {
	...
	public void sendDialogues(DialogueFactory factory) {
		...
		switch (getStage(player)) {
		case 0:
			factory.sendNpcChat(6526, "Greetings adventurer.", "Would you like to partake in my activity?",
					"By completing it you will unlock barrows gloves.").sendOption("Sure", () -> {
						setStage(player, 1);
						factory.clear();
					}, "No, I'm too busy right now",
							() -> factory.sendPlayerChat("No, I'm too busy right now").execute())
					.execute();
			break;
		case 1:
			...
			factory.sendNpcChat(6526, "Click on the portal next to me to enter the activity.",
					"It is safe, items will not be lost on death.",
					"In order to retrieve the highest tier gloves you need",
					"to complete the activity in one inventory").execute();
			break;
		}
	}
	...
	protected boolean clickObject(Player player, ObjectInteractionEvent event) {
		if (event.getOpcode() == 0 && event.getObject().getId() == 4150) {
			...
			if (getState(player) == QuestState.COMPLETED) {
				player.dialogueFactory.sendStatement("You have already completed this activity.",

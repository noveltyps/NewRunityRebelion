...
public abstract class Store {
	 ...
	 * A mapping of each shop by it's name.
	 */
	public static Map<String, PersonalStore> PERSONAL_STORES = new HashMap<>();
	...
	protected static List<PersonalStore> getPersonalShops() {
		List<PersonalStore> personal_stores = new ArrayList<>();
		PERSONAL_STORES.values().stream().forEach(s -> personal_stores.add((PersonalStore) s));
	...
	protected static List<PersonalStore> getFeaturedShops() {
		...
		PERSONAL_STORES.values().stream().filter(s -> s.rank > 0)
	...
	public static void closeShop(Player player) {
		...
		if (store == null) {
			PersonalStore personal = PERSONAL_STORES.get(player.attributes.get("SHOP"));
			if (personal == null)
				return;
			personal.close(player);
			return;
		}
		store.close(player);
	}
	...
	public static void exchange(Player player, int id, int slot, int action, boolean purchase) {
		...
		if (store == null) {
			...
			if (personal == null)
				return;
			personal.itemContainerAction(player, id, slot, action, purchase);
			return;
		}
	...
	public boolean purchase(Player player, Item item, int slot) {
		...
		final Long value = 1L * storeItem.getShopValue();
		
		if (value * item.getAmount() > Integer.MAX_VALUE) {
			player.sendMessage("The total purchace value cannot be over " + Utility.formatDigits(Integer.MAX_VALUE)+".");
			return false;
		}
		
		if (!(type().equals(StoreType.PERSONAL) && isOwner(player)) && !(currencyType.currency.currencyAmount(player) >= (value * item.getAmount()))) {
		...
		if (player.inventory.remaining() >= item.getAmount() && !item.isStackable()
			...
			if (!(type().equals(StoreType.PERSONAL) && isOwner(player))
					&& value > 0 && !currencyType.currency.takeCurrency(player, item.getAmount() * value)) {
				return false;
			}

			if (type().equals(StoreType.PERSONAL) && container.retrieve(slot).isPresent()
					&& (container.retrieve(slot).get().getAmount() - item.getAmount() > 0)) {
	...
	protected final void sell(Player player, Item item, int slot, boolean addX) {
		...
		if (!item.isTradeable() || !inventoryItem.isTradeable()) {
	...
	protected final void sendSellValue(Player player, int slot) {
		...
		final String message = this.sellType() != SellType.NONE ? String.format("[%s] will buy %s for %s %s.", name,
				item.getName(), Utility.formatDigits(value), currencyType.toString())
	...
	protected void sendPurchaseValue(Player player, int slot) {
		...
		if (item instanceof StoreItem) {
			...
			String message = "[" + name + "] will sell " + item.getName() + " for " + (value <= 0 ? "free!"
					: Utility.formatDigits(value) + " " + storeItem.getShopCurrency(this).toString() + ".");
			player.send(new SendMessage(message));
		}
	...
	public static void removePersonalStore(Player player) {
		if (PERSONAL_STORES.containsKey(player.getName()))
			PERSONAL_STORES.remove(player.getName());
	}

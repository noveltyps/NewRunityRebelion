...
public class GenieEvent extends RandomEvent {
	...
	protected boolean clickNpc(Player player, NpcInteractionEvent event) {
		...
		if (event.getOpcode() == 0) {
			...
			player.dialogueFactory.sendNpcChat(id, "Take this lamp as a token of appreciation for playing", "RebelionX.")
					.onAction(() -> {
	...
	protected String[] eventNpcShout() {
		return new String[] { "Hello %name, I'm here to grant you a wish!",
				"Are you there %name? I have something for you!", "C'mon %name, I don't have all day!",

...
public class PlayerRecord {
	...
	public void add(GameRecord tracker) {
		...
		TreeSet<GameRecord> personal = personalRecord.computeIfAbsent(activity,
				act -> new TreeSet<>(Comparator.comparingLong(record -> record.time)));
		if (personal.size() < 10) {
	...
	public void display(boolean global, ActivityType activity) {
		...
		for (GameRecord personal : records) {
			...
			player.send(
					new SendString(personal.time == 0 ? "None!" : Utility.getTime(personal.time), (idx + 3) + 32401));

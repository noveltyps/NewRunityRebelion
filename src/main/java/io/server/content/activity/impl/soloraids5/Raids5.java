...
public class Raids5 extends Activity {
	 ...
	 * {@code instance}.
	 */
	private Raids5(Player player, int instance) {
		super(10, instance);
		this.player = player;
	}

	public static Raids5 create(Player player) {
		Raids5 minigame = new Raids5(player, player.playerAssistant.instance());
		player.move(new Position(1886, 5391, player.getHeight()));
		ActivityPanel.update(player, -1, "Solo Raids 5", "Activity Completion:", "Good Luck, " + player.getName() + "!");
		player.dialogueFactory.sendNpcChat(5567, "Welcome to the Solo Raids 5, #name.",
				"There are a total of 10 waves, we use Challenge Mode order.",
	...
	protected void start() {
		...
		Position spawn = new Position(1886, 5407, player.getHeight());
	...
	public void finish() {
		...
		if (completed) {
			player.dialogueFactory.sendNpcChat(5567, "You have defeated Solo Raids 5, I am most impressed!",
			...
			player.inventory.addOrDrop(new Item(10911));
			}
			player.inventory.addOrDrop(new Item(10911));
			Pets.onReward(player, PetData.PIRATE_PETE);
			player.send(new SendMessage("You have completed the Solo Raids 5 activity. Final time: @red@"
					+ Utility.getTime(time) + "</col>."));
			player.activityLogger.add(ActivityLog.SOLORAIDS5);
			return;
		}
		...
		if (rewards <= 0)
			rewards = 1;
        player.inventory.addOrDrop(new Item(621, rewards));
		player.message("<img=9>You now have @red@" + rewards + " Solo Raids 5 Tickets!");
	...
	public void update() {
		if (wave == null) {
			ActivityPanel.update(player, 100, "Solo Raids 5", new Item(22325), "Congratulations, you have",
					"completed the Solo Raids 5", "activity!");
			return;
		}
		int progress = (int) Utility.getPercentageAmount(wave.ordinal() + 1, Raids5Data.WaveData.values().length);
		if (progress >= 100 && !completed)
			progress = 99;
		ActivityPanel.update(player, progress, "Solo Raids 5", new Item(22325),
	...
	public ActivityType getType() {
		return ActivityType.SOLORAIDS5;
	}

package io.server.content.activity.impl.arcademinigame;
...
public class ArcadeMinigame extends Activity {
	...
	private ArcadeMinigameWaveData.WaveData wave = ArcadeMinigameWaveData.WaveData.WAVE_1;

	/** The combat listener to add for all mobs. */
	private final ArcadeMinigameCavesListener listener = new ArcadeMinigameCavesListener(this);

	/**
	 * Constructs a new {@code AllVsOne} object for a {@code player} and an
	 * {@code instance}.
	 */
	public ArcadeMinigame(Player player, int instance, Player other) {
		super(10, instance);
		this.player = player;
		this.other = other;
	}
	
	public ArcadeMinigame(Player player, int instance, List<Player> partners) {
		super(10, instance);
		this.player = player;
		this.partners = partners;
	}
	
	public static ArcadeMinigame create(Player player, List<Player> partners) {

		ArcadeMinigame minigame = new ArcadeMinigame(player, player.playerAssistant.instance(), partners);

		player.move(new Position(2273, 5341, player.getHeight()));
		
		ActivityPanel.update(player, -1, "Arcade Minigame", "Activity Completion:", "Good Luck, " + player.getName() + "!");
		player.dialogueFactory.sendNpcChat(5567, "Welcome to the Arcade Minigame, #name.",
				"There are a total of X waves",
		...
		for (Player p : partners) {
			...
			ActivityPanel.update(p, -1, "Arcade Minigame", "Activity Completion:", "Good Luck, " + p.getName() + "!");
			
			p.dialogueFactory.sendNpcChat(5567, "Welcome to the Arcade Minigame, #name.",
					"There are a total of X waves",
					"Use your activity panel (bottom left tab) for wave information.", "Good luck!").execute();	
	...
	public static ArcadeMinigame create(Player player, Player other) {
		ArcadeMinigame minigame = new ArcadeMinigame(player, player.playerAssistant.instance(), other);
		...
		ActivityPanel.update(other, -1, "Arcade Minigame", "Activity Completion:", "Good Luck, " + other.getName() + "!");
		other.dialogueFactory.sendNpcChat(5567, "Welcome to the Arcade Minigame, #name.",
				"There are a total of 6 waves",
				"Use your activity panel (bottom left tab) for wave information.", "Good luck!").execute();
	...
	void handleDeath(Mob dead) {
		if (partners.contains(dead)) {
			dead.getPlayer().inventory.addOrDrop(new Item(3, rewards / 2));
		...
		if (dead.isPlayer() && dead.equals(player)) {
			player.inventory.addOrDrop(new Item(3, rewards / 2));
...
//					roc.getCombat().attack(other);
			...
			if (npcs.isEmpty()) {
				wave = ArcadeMinigameWaveData.WaveData.getNext(wave.ordinal());
				if (wave == null) {
					completed = true;
					if (player != null)
...
//		}
	...
	public void finish() {
		...
		if (completed) {
			...
			if (player != null)
				player.dialogueFactory.sendNpcChat(5567, "You have defeated Arcade Minigame, I am most impressed!",
						"Please accept this gift, young thug.").execute();
			
			for (Player p : partners)
				p.dialogueFactory.sendNpcChat(5567, "You have defeated Arcade Minigame, I am most impressed!",
						"Please accept this gift, young thug.").execute();
				
//			other.dialogueFactory.sendNpcChat(5567, "You have defeated Arcade Minigame, I am most impressed!",
//					"Please accept this gift, young thug.").execute();
			
			
			
			if (player != null) {
				player.inventory.addOrDrop(new Item(3, 20000));
	    		player.message("<img=8>You now have @red@" + rewards + " Arcade Minigame Tickets!");
				//player.inventory.addOrDrop(new Item(10993, 1));
				player.send(new SendMessage("You have completed the Arcade Minigame activity. Final time: @red@"
						+ Utility.getTime(time) + "</col>."));
				player.add(player.getarcadePoints() + 1000);
			}
			
			for (Player p : partners) {
				p.inventory.addOrDrop(new Item(3, 20000));
				p.message("<img=8>You now have @red@" + rewards + " Arcade Minigame Tickets!");
				//p.inventory.addOrDrop(new Item(10993, 1));
				p.send(new SendMessage("You have completed the Arcade Minigame activity. Final time: @red@"
						+ Utility.getTime(time) + "</col>."));
				player.add(player.getarcadePoints() + 1000);
			}
				
//            other.inventory.addOrDrop(new Item(3, rewards));

//    		other.message("<img=8>You now have @red@" + rewards + " Arcade Minigame Tickets!");

//			other.inventory.addOrDrop(new Item(10993, 1));
			
//			other.send(new SendMessage("You have completed the Arcade Minigame activity. Final time: @red@"
//					+ Utility.getTime(time) + "</col>."));
			
			player.activityLogger.add(ActivityLog.ARCADEMINIGAME);
			player.getRaidPartners().clear();
			
			
			for (Player p : partners) {
				p.activityLogger.add(ActivityLog.ARCADEMINIGAME);
				p.getRaidPartners().clear();
			}
			
//			other.activityLogger.add(ActivityLog.ArcadeMinigame);
...
//        other.inventory.addOrDrop(new Item(3, rewards / 2));
...
//		other.setAllForOnePartner(null);
	...
	public void update() {
		if (wave == null) {
			if (player != null)
				ActivityPanel.update(player, 100, "Arcade Minigame", new Item(11642), "Congratulations, you have",
						"completed the Arcade Minigame", "activity!");
			for (Player p : partners)
				ActivityPanel.update(p, 100, "Arcade Minigame", new Item(11642), "Congratulations, you have",
						"completed the Arcade Minigame", "activity!");
			return;
		}
		int progress = (int) Utility.getPercentageAmount(wave.ordinal() + 1, ArcadeMinigameWaveData.WaveData.values().length);
		if (progress >= 100 && !completed)
			progress = 99;
		if (player != null)
			ActivityPanel.update(player, progress, "Arcade Minigame", new Item(11642),
					"</col>Wave: <col=FF5500>" + (wave.ordinal() + 1) + "/" + (ArcadeMinigameWaveData.WaveData.values().length),
		...
		for (Player p : partners)
			ActivityPanel.update(p, progress, "Arcade Minigame", new Item(11642),
					"</col>Wave: <col=FF5500>" + (wave.ordinal() + 1) + "/" + (ArcadeMinigameWaveData.WaveData.values().length),
					"</col>Monsters Left: <col=FF5500>" + npcs.size(),
					"</col>Points Gained: <col=FF5500>" + Utility.formatDigits(rewards),
					"</col>Time: <col=FF5500>" + Utility.getTime());
...
//		other.setAllForOnePartner(null);
	...
	public ActivityType getType() {
		return ActivityType.ARCADEMINIGAME;
	}

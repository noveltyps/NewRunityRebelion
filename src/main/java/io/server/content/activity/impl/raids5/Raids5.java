...
public class Raids5 extends Activity {
	...
	private Raids5Data.WaveData wave = Raids5Data.WaveData.WAVE_1;

	/** The combat listener to add for all mobs. */
	private final Raids5CavesListener listener = new Raids5CavesListener(this);

	/**
	 * Constructs a new {@code AllVsOne} object for a {@code player} and an
	 * {@code instance}.
	 */
	public Raids5(Player player, int instance, Player other) {
		super(10, instance);
		this.player = player;
		this.other = other;
	}
	
	public Raids5(Player player, int instance, List<Player> partners) {
		super(10, instance);
		this.player = player;
		this.partners = partners;
	}
	
	public static Raids5 create(Player player, List<Player> partners) {

		Raids5 minigame = new Raids5(player, player.playerAssistant.instance(), partners);

		player.move(new Position(1886, 5391, player.getHeight()));
		
		ActivityPanel.update(player, -1, "Raids 5", "Activity Completion:", "Good Luck, " + player.getName() + "!");
		player.dialogueFactory.sendNpcChat(5567, "Welcome to the Raids 5, #name.",
		...
		for (Player p : partners) {
			...
			ActivityPanel.update(p, -1, "Raids 5", "Activity Completion:", "Good Luck, " + p.getName() + "!");
			
			p.dialogueFactory.sendNpcChat(5567, "Welcome to the Raids 5, #name.",
					"There are a total of 6 waves",
					"Use your activity panel (bottom left tab) for wave information.", "Good luck!").execute();	
	...
	public static Raids5 create(Player player, Player other) {
		Raids5 minigame = new Raids5(player, player.playerAssistant.instance(), other);
		...
		ActivityPanel.update(other, -1, "Raids 5", "Activity Completion:", "Good Luck, " + other.getName() + "!");
		other.dialogueFactory.sendNpcChat(5567, "Welcome to the Raids 5, #name.",
				"There are a total of 6 waves",
				"Use your activity panel (bottom left tab) for wave information.", "Good luck!").execute();
...
//					roc.getCombat().attack(other);
			...
			if (npcs.isEmpty()) {
				wave = Raids5Data.WaveData.getNext(wave.ordinal());
				if (wave == null) {
					completed = true;
					if (player != null)
...
//		}
	...
	public void finish() {
		...
		if (completed) {
			...
			if (player != null)
				player.dialogueFactory.sendNpcChat(5567, "You have defeated Raids 5, I am most impressed!",
						"Please accept this gift, young thug.").execute();
			
			for (Player p : partners)
				p.dialogueFactory.sendNpcChat(5567, "You have defeated Raids 5, I am most impressed!",
						"Please accept this gift, young thug.").execute();
				
//			other.dialogueFactory.sendNpcChat(5567, "You have defeated Raids 5, I am most impressed!",
//					"Please accept this gift, young thug.").execute();
			...
			if (player != null) {
	    		...
	    		player.message("<img=8>You now have @red@" + rewards + " Raids 5 Tickets!");
				player.inventory.addOrDrop(new Item(10912, 1));
				player.send(new SendMessage("You have completed the Raids 5 activity. Final time: @red@"
						+ Utility.getTime(time) + "</col>."));
			}
			
			for (Player p : partners) {
				p.inventory.addOrDrop(new Item(621, rewards));
				p.message("<img=8>You now have @red@" + rewards + " Raids 5 Tickets!");
				p.inventory.addOrDrop(new Item(10912, 1));
				p.send(new SendMessage("You have completed the Raids 5 activity. Final time: @red@"
						+ Utility.getTime(time) + "</col>."));
			}
				
//            other.inventory.addOrDrop(new Item(621, rewards));

//    		other.message("<img=8>You now have @red@" + rewards + " Raids 5 Tickets!");

//			other.inventory.addOrDrop(new Item(10993, 1));
			
//			other.send(new SendMessage("You have completed the Raids 5 activity. Final time: @red@"
//					+ Utility.getTime(time) + "</col>."));
			
			player.activityLogger.add(ActivityLog.RAIDS5);
			player.getRaidPartners().clear();
			
			
			for (Player p : partners) {
				p.activityLogger.add(ActivityLog.RAIDS5);
				p.getRaidPartners().clear();
			}
			
//			other.activityLogger.add(ActivityLog.raids5);
...
//		other.setAllForOnePartner(null);
	...
	public void update() {
		if (wave == null) {
			if (player != null)
				ActivityPanel.update(player, 100, "Raids 5", new Item(11642), "Congratulations, you have",
						"completed the Raids 5", "activity!");
			for (Player p : partners)
				ActivityPanel.update(p, 100, "Raids 5", new Item(11642), "Congratulations, you have",
						"completed the Raids 5", "activity!");
			return;
		}
		int progress = (int) Utility.getPercentageAmount(wave.ordinal() + 1, Raids5Data.WaveData.values().length);
		if (progress >= 100 && !completed)
			progress = 99;
		if (player != null)
			ActivityPanel.update(player, progress, "Raids 5", new Item(11642),
					"</col>Wave: <col=FF5500>" + (wave.ordinal() + 1) + "/" + (Raids5Data.WaveData.values().length),
		...
		for (Player p : partners)
			ActivityPanel.update(p, progress, "Raids 5", new Item(11642),
					"</col>Wave: <col=FF5500>" + (wave.ordinal() + 1) + "/" + (Raids5Data.WaveData.values().length),
					"</col>Monsters Left: <col=FF5500>" + npcs.size(),
					"</col>Points Gained: <col=FF5500>" + Utility.formatDigits(rewards),
					"</col>Time: <col=FF5500>" + Utility.getTime());

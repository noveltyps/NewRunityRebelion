...
public class Raids4 extends Activity {
	 ...
	 * {@code instance}.
	 */
	private Raids4(Player player, int instance) {
		super(10, instance);
		this.player = player;
	}

	public static Raids4 create(Player player) {
		Raids4 minigame = new Raids4(player, player.playerAssistant.instance());
		...
		player.dialogueFactory.sendNpcChat(5567, "Welcome to the Solo Raids 4, #name.",
	...
	protected void start() {
		...
		Position spawn = new Position(2765, 9322, player.getHeight());
	...
	public void finish() {
		...
		if (completed) {
			player.dialogueFactory.sendNpcChat(5567, "You have defeated Solo Raids 4, I am most impressed!",
			...
			player.inventory.addOrDrop(new Item(22127));
			}
			player.inventory.addOrDrop(new Item(22127));
			Pets.onReward(player, PetData.PIRATE_PETE);
			player.send(new SendMessage("You have completed the Solo Raids 4 activity. Final time: @red@"
		...
		if (rewards <= 0)
			rewards = 1;
        player.inventory.addOrDrop(new Item(621, rewards));
		player.message("<img=9>You now have @red@" + rewards + " Solo Raids 4 Tickets!");
	...
	public void update() {
		if (wave == null) {
			ActivityPanel.update(player, 100, "Solo Raids 4", new Item(22325), "Congratulations, you have",
					"completed the Solo Raids 4", "activity!");
			return;
		}
		int progress = (int) Utility.getPercentageAmount(wave.ordinal() + 1, Raids4Data.WaveData.values().length);
		if (progress >= 100 && !completed)
			progress = 99;
		ActivityPanel.update(player, progress, "Solo Raids 4", new Item(22325),

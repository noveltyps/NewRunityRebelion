...
public class PestControl extends GroupActivity {
	...
	public static void sequenceMinigame() {
		...
		while (iterator.hasNext()) {
			...
			if (!pestControl.lobby) {
				if (pestControl.getTicks() == START || pestControl.getActiveSize() == 0
						|| pestControl.portalSet.isEmpty()) {
					pestControl.finish();
	...
	public void sequence() {
		...
		forEachActivity((mob, activity) -> activity.getPanel()
				.ifPresent(panel -> ((PestControlPanel) panel).update((PestControlNode) activity)));
	}

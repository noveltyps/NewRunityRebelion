...
public class Raids2 extends Activity {
	...
	private Raids2Data.WaveData wave = Raids2Data.WaveData.WAVE_1;

	/** The combat listener to add for all mobs. */
	private final Raids2CavesListener listener = new Raids2CavesListener(this);

	/**
	 * Constructs a new {@code AllVsOne} object for a {@code player} and an
	 * {@code instance}.
	 */
	public Raids2(Player player, int instance, Player other) {
		super(10, instance);
		this.player = player;
		this.other = other;
	}
	
	public Raids2(Player player, int instance, List<Player> partners) {
		super(10, instance);
		this.player = player;
		this.partners = partners;
	}
	
	public static Raids2 create(Player player, List<Player> partners) {

		Raids2 minigame = new Raids2(player, player.playerAssistant.instance(), partners);

		player.move(new Position(3168, 4321, player.getHeight()));
		
		ActivityPanel.update(player, -1, "Raids 2", "Activity Completion:", "Good Luck, " + player.getName() + "!");
		player.dialogueFactory.sendNpcChat(5567, "Welcome to the Raids 2, #name.",
		...
		for (Player p : partners) {
			...
			ActivityPanel.update(p, -1, "Raids 2", "Activity Completion:", "Good Luck, " + p.getName() + "!");
			
			p.dialogueFactory.sendNpcChat(5567, "Welcome to the Raids 2, #name.",
					"There are a total of 6 waves",
					"Use your activity panel (bottom left tab) for wave information.", "Good luck!").execute();	
	...
	public static Raids2 create(Player player, Player other) {
		Raids2 minigame = new Raids2(player, player.playerAssistant.instance(), other);
		...
		ActivityPanel.update(other, -1, "Raids 2", "Activity Completion:", "Good Luck, " + other.getName() + "!");
		other.dialogueFactory.sendNpcChat(5567, "Welcome to the Raids 2, #name.",
				"There are a total of 6 waves",
				"Use your activity panel (bottom left tab) for wave information.", "Good luck!").execute();
...
//					roc.getCombat().attack(other);
			...
			if (npcs.isEmpty()) {
				wave = Raids2Data.WaveData.getNext(wave.ordinal());
				if (wave == null) {
					completed = true;
					if (player != null)
...
//		}
	...
	public void finish() {
		...
		if (completed) {
			...
			if (player != null)
				player.dialogueFactory.sendNpcChat(5567, "You have defeated Raids 2, I am most impressed!",
						"Please accept this gift, young thug.").execute();
			
			for (Player p : partners)
				p.dialogueFactory.sendNpcChat(5567, "You have defeated Raids 2, I am most impressed!",
						"Please accept this gift, young thug.").execute();
				
//			other.dialogueFactory.sendNpcChat(5567, "You have defeated Raids 2, I am most impressed!",
//					"Please accept this gift, young thug.").execute();
			...
			if (player != null) {
	    		...
	    		player.message("<img=8>You now have @red@" + rewards + " Raids 2 Tickets!");
				player.inventory.addOrDrop(new Item(10993, 1));
				player.send(new SendMessage("You have completed the Raids 2 activity. Final time: @red@"
						+ Utility.getTime(time) + "</col>."));
			}
			
			for (Player p : partners) {
				p.inventory.addOrDrop(new Item(621, rewards));
				p.message("<img=8>You now have @red@" + rewards + " Raids 2 Tickets!");
				p.inventory.addOrDrop(new Item(10993, 1));
				p.send(new SendMessage("You have completed the Raids 2 activity. Final time: @red@"
						+ Utility.getTime(time) + "</col>."));
			}
				
//            other.inventory.addOrDrop(new Item(621, rewards));

//    		other.message("<img=8>You now have @red@" + rewards + " Raids 2 Tickets!");

//			other.inventory.addOrDrop(new Item(10993, 1));
			
//			other.send(new SendMessage("You have completed the Raids 2 activity. Final time: @red@"
//					+ Utility.getTime(time) + "</col>."));
			
			player.activityLogger.add(ActivityLog.RAIDS2);
			player.getRaidPartners().clear();
			
			
			for (Player p : partners) {
				p.activityLogger.add(ActivityLog.RAIDS2);
				p.getRaidPartners().clear();
			}
			
//			other.activityLogger.add(ActivityLog.RAIDS2);
...
//		other.setAllForOnePartner(null);
	...
	public void update() {
		if (wave == null) {
			if (player != null)
				ActivityPanel.update(player, 100, "Raids 2", new Item(11642), "Congratulations, you have",
						"completed the Raids 2", "activity!");
			for (Player p : partners)
				ActivityPanel.update(p, 100, "Raids 2", new Item(11642), "Congratulations, you have",
						"completed the Raids 2", "activity!");
			return;
		}
		int progress = (int) Utility.getPercentageAmount(wave.ordinal() + 1, Raids2Data.WaveData.values().length);
		if (progress >= 100 && !completed)
			progress = 99;
		if (player != null)
			ActivityPanel.update(player, progress, "Raids 2", new Item(11642),
					"</col>Wave: <col=FF5500>" + (wave.ordinal() + 1) + "/" + (Raids2Data.WaveData.values().length),
		...
		for (Player p : partners)
			ActivityPanel.update(p, progress, "Raids 2", new Item(11642),
					"</col>Wave: <col=FF5500>" + (wave.ordinal() + 1) + "/" + (Raids2Data.WaveData.values().length),
					"</col>Monsters Left: <col=FF5500>" + npcs.size(),
					"</col>Points Gained: <col=FF5500>" + Utility.formatDigits(rewards),
					"</col>Time: <col=FF5500>" + Utility.getTime());

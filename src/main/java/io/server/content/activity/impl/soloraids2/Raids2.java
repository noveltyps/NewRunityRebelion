...
public class Raids2 extends Activity {
	 ...
	 * {@code instance}.
	 */
	private Raids2(Player player, int instance) {
		super(10, instance);
		this.player = player;
	}

	public static Raids2 create(Player player) {
		Raids2 minigame = new Raids2(player, player.playerAssistant.instance());
		player.move(new Position(3168, 4321, player.getHeight()));
		ActivityPanel.update(player, -1, "Solo Raids 2", "Activity Completion:", "Good Luck, " + player.getName() + "!");
	...
	public void finish() {
		...
		if (completed) {
			player.dialogueFactory.sendNpcChat(5567, "You have defeated Solo Raids 2, I am most impressed!",
			...
			player.inventory.addOrDrop(new Item(10993));
			}
			player.inventory.addOrDrop(new Item(10993));
			Pets.onReward(player, PetData.PIRATE_PETE);
			player.send(new SendMessage("You have completed the Solo Raids 2 activity. Final time: @red@"
	...
	public void update() {
		if (wave == null) {
			ActivityPanel.update(player, 100, "Solo Raids 2", new Item(22325), "Congratulations, you have",
					"completed the Solo Raids 2", "activity!");
			return;
		}
		int progress = (int) Utility.getPercentageAmount(wave.ordinal() + 1, Raids2Data.WaveData.values().length);
		if (progress >= 100 && !completed)
			progress = 99;
		ActivityPanel.update(player, progress, "Solo Raids 2", new Item(22325),

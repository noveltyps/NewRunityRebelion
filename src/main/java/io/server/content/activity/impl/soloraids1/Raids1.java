...
public class Raids1 extends Activity {
	...
	private Raids1Data.WaveData wave = Raids1Data.WaveData.WAVE_1;

	/** The combat listener to add for all mobs. */
	private final Raids1CavesListener listener = new Raids1CavesListener(this);

	/**
	 * Constructs a new {@code Raids1} object for a {@code player} and an
	 * {@code instance}.
	 */
	private Raids1(Player player, int instance) {
		super(10, instance);
		this.player = player;
	}

	public static Raids1 create(Player player) {
		Raids1 minigame = new Raids1(player, player.playerAssistant.instance());
		player.move(new Position(3342, 5273, player.getHeight()));
		ActivityPanel.update(player, -1, "Solo Raids 1", "Activity Completion:", "Good Luck, " + player.getName() + "!");
	...
	void handleDeath(Mob dead) {
		...
		if (dead.isNpc() && npcs.contains(dead)) {
			...
			if (npcs.isEmpty()) {
				wave = Raids1Data.WaveData.getNext(wave.ordinal());
				if (wave == null) {
					completed = true;
	...
	protected void start() {
		...
		Position spawn = new Position(3343, 5262, player.getHeight());
	...
	public void finish() {
		...
		if (completed) {
			player.dialogueFactory.sendNpcChat(5567, "You have defeated Solo Raids 1, I am most impressed!",
    		...
    		player.message("<img=9>You now have @red@" + rewards + " Solo Raids 1 Tickets!");
			...
			player.send(new SendMessage("You have completed the Solo Raids 1 activity. Final time: @red@"
	...
	public void update() {
		if (wave == null) {
			ActivityPanel.update(player, 100, "Solo Raids 1", new Item(22325), "Congratulations, you have",
					"completed the Solo Raids 1", "activity!");
			return;
		}
		int progress = (int) Utility.getPercentageAmount(wave.ordinal() + 1, Raids1Data.WaveData.values().length);
		if (progress >= 100 && !completed)
			progress = 99;
		ActivityPanel.update(player, progress, "Solo Raids 1", new Item(22325),
				"</col>Wave: <col=FF5500>" + (wave.ordinal() + 1) + "/" + (Raids1Data.WaveData.values().length),

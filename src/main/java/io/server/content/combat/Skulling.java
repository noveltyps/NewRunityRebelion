package io.server.content.combat;

import java.util.LinkedList;
import java.util.List;

import io.server.Config;
import io.server.game.task.impl.SkullRemoveTask;
...
public class Skulling {
	...
	public void skull(Player attacking, SkullHeadIconType icon) {
		...
		if (!skullRemoveTask.isRunning()) {
			skullRemoveTask.setSkullTime(
					skullRemoveTask.getSkullTime() <= 0 ? Config.SKULL_TIME : skullRemoveTask.getSkullTime());

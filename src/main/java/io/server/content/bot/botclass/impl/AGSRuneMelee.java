...
import static io.server.game.world.entity.combat.attack.FormulaFactory.getModifiedMaxHit;
...
public class AGSRuneMelee extends SimplifiedListener<Player> implements BotClass {
	...
	public Item[] equipment() {
		return new Item[] { new Item(10828), new Item(10551), new Item(1079), new Item(3105), new Item(4587),
	...
	public void hit(Player attacker, Mob defender, Hit hit) {
		...
		boolean lowHp = defender.getCurrentHealth() <= defender.getMaximumHealth() * RandomUtils.inclusive(0.50, 0.45);
		boolean combo = defender.getCurrentHealth() <= defender.getMaximumHealth() * RandomUtils.inclusive(0.50, 0.55)
				&& hit.getDamage() >= max * RandomUtils.inclusive(0.45, 0.55);
		...
		bot.schedule(4, () -> {
			...
			if (hasSpec && (lowHp || RandomUtils.success(0.45)))
	...
	public void pot(Player target, PlayerBot bot) {
		if (target.getCurrentHealth() <= Math.floor(target.getMaximumHealth() * 0.15))
			return;

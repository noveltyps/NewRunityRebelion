...
import static io.server.content.clanchannel.channel.ClanManagement.ENTER_RANK_INDEX;
import static io.server.content.clanchannel.channel.ClanManagement.MANAGE_RANK_INDEX;
import static io.server.content.clanchannel.channel.ClanManagement.TALK_RANK_INDEX;
...
public class ClanChannelHandler {
	...
	public static void connect(Player player, String owner, boolean fromLogin) {
		...
		if (channel == null) {
			...
			for (File file : files) {
				if (file.getName().toLowerCase().contains(player.getUsername().toLowerCase().trim())
						&& file.length() > 0) {
					loaded = false;
					break;
				}
	...
	public static void manage(Player player) {
		...
		member.ifPresent(m -> {
			...
			player.send(new SendString(channel.getDetails().type.getIcon() + "" + channel.getDetails().type.getName(),
					42110));
	...
	void connected(ClanMember member) {
		...
		member.player.ifPresent(player -> {
			player.send(new SendMessage("Now talking in clan chat <col=FFFF64><shad=0>"
					+ Utility.formatName(channel.getName()) + "</shad></col>."));
	...
	void sendConnectionWarning(Player player, ClanMember member) {
		player.dialogueFactory
				.sendStatement("-<col=FF0000>WARNING</col>-", "Contributing to this clan or gaining a rank will remove",
						"all of your progress from your current clan <col=FF4444>" + player.lastClan + "</col>.",
						"Are you sure you want to join?")
				.sendOption("Yes, I accept.", () -> channel.add(player, member), "No, I don't want to risk it.",
						() -> player.clanChannel = null)
				.execute();
	}
	...
	public static void manageMember(Player player, int button) {
		...
		channel.getMember(player.getName()).ifPresent(member -> {
			...
			if (other != null && !other.name.equalsIgnoreCase(player.getName())
		...
		for (int index = 0; index < size * 3; index++) {
			...
			if (nextMember == null || (!nextMember.player.isPresent() && index / 3 >= members.size())) {
				...
				player.send(new SendTooltip(
						tooltip ? (nextMember.name.equals(player.getName()) ? "" : "Manage " + nextMember.name) : "",
						string++));
	...
	public static void connect(Player player, String string) {

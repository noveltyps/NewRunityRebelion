...
public class Pets {
	...
	public final static int INSRUANCE_COST = 10000000;
...
//        System.out.println(String.format("item=%s skill=%s base=%s change=%s", item, skill, base, change));
	...
	public static boolean onSpawn(Player player, int item, boolean drop) {
		if (!PetData.forItem(item).isPresent())
			return false;
		if (Area.inWilderness(player) || Area.inEventArena(player) || Area.inTournamentArena(player)
				|| player.getY() == 3523 || player.getY() == 3524) {
		...
		if (!drop) {
			...
			World.sendPetMessage(player, pet);
		}
		return true;
	}
	...
	public static void buyInsurance(Player player, Item item) {
		...
		if (!petData.isPresent()) {
			player.dialogueFactory.sendNpcChat(7601, "I can't insure " + item.getName() + "!",
					"What do I look like to you?", "Show me some respect and use a pet on me.").execute();
			return;
		}
		if (hasInsurance(player, petData.get())) {
			player.dialogueFactory.sendNpcChat(7601, "You already have insurance for " + petData.get().getName() + "!")
					.execute();
			return;
		}
		if (!player.inventory.contains(995, INSRUANCE_COST)) {
			player.dialogueFactory
					.sendNpcChat(7601, "Naw, it's going to cost " + Utility.formatDigits(INSRUANCE_COST) + " coins",
							"to insure your pet. Start hustling fam.")
					.execute();
			return;
		}
		player.inventory.remove(995, INSRUANCE_COST);
		player.petInsurance.add(petData.get());
		player.dialogueFactory.sendNpcChat(7601, "You can put your trust in my that I will",
				"protect your little " + petData.get().getName() + ".").execute();
	}
	...
	public static void claimLostPets(Player player) {
		...
		if (!player.inventory.contains(995, cost)) {
			player.dialogueFactory.sendNpcChat(7601, "You need " + Utility.formatDigits(cost) + " coins to claim all",
					"of your lost pets.");
			return;
		}
		if (player.inventory.getFreeSlots() < player.lostPets.size()) {
			player.dialogueFactory.sendNpcChat(7601, "You need " + player.lostPets.size() + " free inventory spaces",
					"to claim your pets!");
			return;
		}
		...
		player.dialogueFactory.sendNpcChat(7601,
				"You have claimed all your lost pets for " + Utility.formatDigits(cost) + " coins.");
	}
	...
	public static void openInsurance(Player player) {
		...
		for (int index = 0, string = 37115; index < size; index++) {
			...
			player.send(new SendString(
					"<col=" + (hasInsurance(player, pet) ? "347043" : "F24444") + ">" + pet.getName(), string++));
		}
		player.send(new SendString("<col=000000>This is a list of all pets that are allowed to be insured", 37111));
		player.send(new SendString("<col=000000>for <col=3c50b2>" + Utility.formatDigits(INSRUANCE_COST)
				+ "<col=000000> gp. Insurance will protect a pet on death.", 37112));
		player.send(new SendString("<col=000000>Red = <col=F24444>Un-insured<col=000000> | Green = <col=347043>Insured",
				37113));
	...
	public static void openLostPets(Player player) {
		...
		player.send(new SendString(
				"<col=000000>Total Free <col=3c50b2>" + Utility.formatDigits(cost) + "<col=000000> gp", 37112));

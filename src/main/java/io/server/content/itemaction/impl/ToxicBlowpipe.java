...
public class ToxicBlowpipe extends ItemAction {
	...
	private static void charge(Player player, Item scales) {
		...
		player.send(new SendMessage(
				String.format("You load %s scales into your blowpipe.", Utility.formatDigits(scales.getAmount()))));
	}
	...
	private static void uncharge(Player player) {
		...
		player.send(new SendMessage(String.format("You uncharged your blowpipe and received %s of Zulra's scales.",
				Utility.formatDigits(scales.getAmount()))));
	}
	...
	private static void load(Player player, Item ammo) {
		...
		if (player.blowpipeDarts == null) {
			...
			player.send(new SendMessage(String.format("You load your blowpipe with %s %ss.",
					Utility.formatDigits(ammo.getAmount()), ammo.getName())));
			return;
		}
		...
		player.send(new SendMessage(String.format("You load your blowpipe with %s %s.",
				Utility.formatDigits(ammo.getAmount()), ammo.getName())));
	}

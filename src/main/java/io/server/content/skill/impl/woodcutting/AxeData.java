 ...
 */
public enum AxeData {
	BRONZE(1351, 1, 879, 0.1), IRON(1349, 1, 877, 0.2), STEEL(1353, 6, 875, 0.3), BLACK(1361, 6, 873, 0.4),
	MITHRIL(1355, 21, 871, 0.5), ADAMANT(1357, 31, 869, 0.6), RUNE(1359, 41, 867, 0.75),
	INFERNO_ADZE(13241, 75, 2117, 0.80), THIRD_AGE(20011, 90, 7264, 0.80), DRAGON(6739, 61, 2846, 0.85);
	...
	public static Optional<AxeData> getDefinition(Player player) {
		if (player.equipment.hasWeapon()) {
			Optional<AxeData> result = VALUES.stream().filter(
					it -> player.equipment.contains(it.id) && player.skills.getLevel(Skill.WOODCUTTING) >= it.level)
					.findFirst();

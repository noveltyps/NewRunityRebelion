...
public class House {
	...
	public void purchase() {
		...
		player.dialogueFactory.sendStatement("You have successfully purchased a house for 1,000,000 royale tokens.",
	...
	public void location(BuildableMap newMap) {
		...
		if (player.skills.getLevel(Skill.CONSTRUCTION) < newMap.getLevel()) {
			player.dialogueFactory
					.sendStatement("You need a construction level of " + newMap.getLevel() + " to build this.")
					.execute();
			return;
		}
		...
		if (!player.inventory.contains(new Item(995, newMap.getCost()))) {
			player.dialogueFactory
					.sendStatement("You need " + newMap.getCost() + " royale tokens to purchase this map.").execute();
			return;
		}
		...
		player.dialogueFactory.sendStatement("You have successfully purchased the " + newMap.getName() + " map.")
				.execute();
	}
	...
	public void enter() {
		if (map == null) {
			player.dialogueFactory
					.sendStatement("You need to purchase a house first.", "Speak to the agent to purchase one.")
					.execute();
			return;
		}
	...
	public void enter(String name) {
		...
		if (otherHouse.getMap() == null) {
			player.dialogueFactory.sendStatement(Utility.formatName(name) + " does not have a house for you to visit.")
					.execute();
			return;
		}

		if (!otherHouse.isInside()) {
			player.dialogueFactory.sendStatement(Utility.formatName(name) + " must be in their house for you to visit.")
					.execute();
		}
		...
		player.send(new SendMinimapState(MinimapState.HIDDEN));
		player.interfaceManager.open(12283);
		player.move(new Position(other.house.getMap().getPosition().getX(), otherHouse.getMap().getPosition().getY(),
				otherHouse.getHeight()));
		otherHouse.getMap().handle(player);

		for (int index = 0; index < otherHouse.getObjects().size(); index++) {
			new CustomGameObject(otherHouse.getObjects().get(index).getObject(),
					otherHouse.getObjects().get(index).getPosition()).register();
		}
	...
	public boolean toolkit(GameObject object) {
		...
		for (int index = 0; index < OBJECT.size(); index++) {
			if (OBJECT.get(index).getObject() == object.getId()
					&& OBJECT.get(index).getPosition().equals(object.getPosition())) {
				constructionObject = new ConstructionObject(object.getId(), OBJECT.get(index).getRotation(),
						object.getPosition());
				break;
			}
		}
		...
		player.dialogueFactory.sendOption("Remove object", () -> {
			...
			player.dialogueFactory.onAction(() -> {
				player.dialogueFactory.sendStatement("Please select the rotation direction").sendOption("North", () -> {
					object.unregister();
					new CustomGameObject(object.getId(), object.getPosition(), ObjectDirection.NORTH,
							ObjectType.INTERACTABLE).register();
				}, "East", () -> {
					object.unregister();
					new CustomGameObject(object.getId(), object.getPosition(), ObjectDirection.EAST,
							ObjectType.INTERACTABLE).register();
				}, "West", () -> {
					object.unregister();
					new CustomGameObject(object.getId(), object.getPosition(), ObjectDirection.WEST,
							ObjectType.INTERACTABLE).register();
				}, "South", () -> {
					object.unregister();
					new CustomGameObject(object.getId(), object.getPosition(), ObjectDirection.SOUTH,
							ObjectType.INTERACTABLE).register();
				}, "Nevermind", () -> {
					player.interfaceManager.close();
				}).execute();
	...
	public void construct() {
		...
		World.schedule(5, () -> {
			...
			player.send(new SendMessage("You have successfully constructed " + Utility.getAOrAn(gameObject.getName())
					+ " " + gameObject.getName() + "."));
		});
	}

...
import io.server.content.skill.impl.ProducingSkillAction;
...
import io.server.util.ItemIdentifiers;
...
public final class SmithingArmour extends ProducingSkillAction {
	...
	private static final ImmutableMap<Integer, SmithingTable[]> TABLE = ImmutableMap.<Integer, SmithingTable[]>builder()
			.put(2349, SmithingTable.BronzeTable.values()).put(2351, SmithingTable.IronTable.values())
			.put(2353, SmithingTable.SteelTable.values()).put(2359, SmithingTable.MithrilTable.values())
			.put(2361, SmithingTable.AdamantTable.values()).put(2363, SmithingTable.RuniteTable.values()).build();
	private static final int[][] FRAME_DATA = { { 1125, 1094, 0, 1119 }, { 1126, 1091, 0, 1120 },
			{ 1109, 1098, 0, 1121 }, { 1127, 1102, 0, 1122 }, { 1128, 1107, 0, 1123 }, { 1124, 1085, 1, 1119 },
			{ 1129, 1093, 1, 1120 }, { 1110, 1099, 1, 1121 }, { 1113, 1103, 1, 1122 }, { 1130, 1108, 1, 1123 },
			{ 1116, 1087, 2, 1119 }, { 1118, 1083, 2, 1120 }, { 1111, 1100, 2, 1121 }, { 1114, 1104, 2, 1122 },
			{ 1131, 1106, 2, 1123 }, { 1089, 1086, 3, 1119 }, { 1095, 1092, 3, 1120 }, { 1112, 1101, 3, 1121 },
			{ 1115, 1105, 3, 1122 }, { 1132, 1096, 3, 1123 }, { 1090, 1088, 4, 1119 }, { 8428, 8429, 4, 1120 },
			{ 11459, 11461, 4, 1121 }, { 13357, 13358, 4, 1122 }, { 1135, 1134, 4, 1123 }, };
	...
	static boolean openInterface(Player player, Item item, GameObject object) {
		...
		for (int i = 0; i < FRAME_DATA.length; i++) {
			...
			final boolean has_bar = player.inventory.computeAmountForId(table.getBar().getId()) >= table
					.getBarsRequired();
			final String bar_color = !has_bar ? "@red@" : "@gre@";
			final String name_color = player.skills.get(Skill.SMITHING).getMaxLevel() >= table.getLevelRequirement()
					&& has_bar ? "@whi@" : "@bla@";

			player.send(new SendString(
					bar_color + table.getBarsRequired() + " Bar" + (table.getBarsRequired() != 1 ? "s" : ""),
					FRAME_DATA[i][0]));
			player.send(new SendString(name_color + StringUtils.capitalize(table.getName().toLowerCase()),
					FRAME_DATA[i][1]));
	...
	public boolean canInit() {
		if (!getMob().skills.get(Skill.SMITHING).reqLevel(definition.getLevelRequirement())) {
			getMob().getPlayer().send(new SendMessage("You need a smithing level of " + definition.getLevelRequirement()
					+ " to smith " + StringUtils.appendIndefiniteArticle(definition.getName())));
			return false;
		}
		return true;
	}

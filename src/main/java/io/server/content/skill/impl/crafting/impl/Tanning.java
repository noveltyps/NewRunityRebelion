...
public class Tanning {
	 ...
	 */
	public enum TanData {
	LEATHER(1739, 1741, 1), HARD_LEATHER(1739, 1743, 3), SNAKE_HIDE(6287, 6287, 15), SNAKESKIN(6287, 6289, 20),
	GREEN_LEATHER(1753, 1745, 20), BLUE_LEATHER(1751, 2505, 20), RED_LEATHER(1749, 2507, 20),
	...
	public static void open(Player player) {
		...
		for (TanData data : TanData.values()) {
			...
			player.send(new SendString((player.inventory.contains(data.ingredient) ? "<col=23db44>" : "<col=e0061c>")
					+ Utility.formatEnum(data.name()), 14777 + count));
			player.send(new SendString(
					(player.inventory.contains(new Item(Config.CURRENCY, data.cost)) ? "<col=23db44>" : "<col=e0061c>")
							+ data.cost + " coins",
					14785 + count));
			count++;
		}
	...
	public static void tan(Player player, int amount, TanData data) {
		if (!player.inventory.contains(data.ingredient)) {
			player.send(new SendMessage(
					"You do not have any " + ItemDefinition.get(data.ingredient).getName() + " to do this."));
			return;
		}
		...
		if (!player.inventory.contains(new Item(Config.CURRENCY, cost))) {
			player.send(new SendMessage("You need " + cost + " coins to tan " + amount + " "
					+ ItemDefinition.get(data.ingredient).getName() + "!"));
			return;
		}
		...
		player.send(new SendMessage("You successfully tan all the " + ItemDefinition.get(data.ingredient).getName()
				+ " for " + amount + " coins."));
	}

package io.server.content.skill.impl.crafting;
...
import io.server.content.skill.impl.crafting.impl.Glass;
import io.server.content.skill.impl.crafting.impl.Jewellery;
import io.server.content.skill.impl.crafting.impl.Leather;
import io.server.content.skill.impl.crafting.impl.Spinning;
import io.server.content.skill.impl.crafting.impl.Stringing;
...
public class Crafting extends Skill {
	...
	private static final Object[][] LEATHER_ARMOR_IDS = { { 8635, 1, Leather.LEATHER_BODY },
			{ 8634, 5, Leather.LEATHER_BODY }, { 8633, 10, Leather.LEATHER_BODY }, { 8638, 1, Leather.LEATHER_GLOVES },
			{ 8637, 5, Leather.LEATHER_GLOVES }, { 8636, 10, Leather.LEATHER_GLOVES },
			{ 8641, 1, Leather.LEATHER_BOOTS }, { 8640, 5, Leather.LEATHER_BOOTS }, { 8639, 10, Leather.LEATHER_BOOTS },
			{ 8644, 1, Leather.LEATHER_VANBRACES }, { 8643, 5, Leather.LEATHER_VANBRACES },
			{ 8642, 10, Leather.LEATHER_VANBRACES }, { 8647, 1, Leather.LEATHER_CHAPS },
			{ 8646, 5, Leather.LEATHER_CHAPS }, { 8645, 10, Leather.LEATHER_CHAPS }, { 8650, 1, Leather.LEATHER_COIF },
			{ 8649, 5, Leather.LEATHER_COIF }, { 8648, 10, Leather.LEATHER_COIF }, { 8653, 1, Leather.LEATHER_COWL },
			{ 8652, 5, Leather.LEATHER_COWL }, { 8651, 10, Leather.LEATHER_COWL }, };
	...
	public static void addCraftable(Craftable craftable) {
		if (CRAFTABLES.put(craftable.getWith().getId(), craftable) != null) {
			System.out.println("[Crafting] Conflicting item values: " + craftable.getWith().getId() + " Type: "
					+ craftable.getName());
		}
	}
	...
	protected boolean useItem(Player player, ItemOnItemInteractionEvent event) {
		...
		if (!craftable.getUse().equalIds(first) && !craftable.getUse().equalIds(second)) {
			player.message("You need to use this with " + Utility.getAOrAn(craftable.getUse().getName()) + " "
					+ craftable.getUse().getName().toLowerCase() + " to craft this item.");
			return true;
		}
	...
	protected boolean clickButton(Player player, ClickButtonInteractionEvent event) {
		...
		case 8946:
			player.send(new SendInputAmount("Enter amount", 10,
					input -> start(player, craftable, 0, Integer.parseInt(input))));
			return true;
		...
		case 8950:
			player.send(new SendInputAmount("Enter amount", 10,
					input -> start(player, craftable, 1, Integer.parseInt(input))));
			return true;
		...
		case 8954:
			player.send(new SendInputAmount("Enter amount", 10,
					input -> start(player, craftable, 2, Integer.parseInt(input))));
			return true;
		...
		case 8958:
			player.send(new SendInputAmount("Enter amount", 10,
					input -> start(player, craftable, 3, Integer.parseInt(input))));
			return true;
		...
		case 8962:
			player.send(new SendInputAmount("Enter amount", 10,
					input -> start(player, craftable, 4, Integer.parseInt(input))));
			return true;
	...
	public boolean start(Player player, Craftable craftable, int index, int amount) {
		...
		if (player.skills.getLevel(Skill.CRAFTING) < item.getLevel()) {
			player.dialogueFactory
					.sendStatement("<col=369>You need a Crafting level of " + item.getLevel() + " to do that.")
					.execute();
			return true;
		}
		...
		if (!player.inventory.containsAll(craftable.getIngediants(index))) {
			...
			player.send(new SendMessage("You need " + requiredItem.getAmount() + " piece"
					+ (requiredItem.getAmount() > 1 ? "s" : "") + " of " + requiredItem.getName().toLowerCase()
					+ " to make" + productAmount + " " + product.getName().toLowerCase() + "."));
			return true;
		}

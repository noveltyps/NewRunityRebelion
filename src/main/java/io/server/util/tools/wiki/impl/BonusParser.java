...
public class BonusParser extends WikiTableParser {
	...
	/** The table links. */
	private static final String[] TABLE_LINKS = { "http://oldschoolrunescape.wikia.com/wiki/Ammunition_slot_table",
							...
							while (iterator.hasNext()) {
								...
								try {
									...
									for (Element data : bonuses) {
										...
										if ((link.contains("Neck") || link.contains("Body") || link.contains("Leg")
												|| link.contains("Feet") || link.contains("Head")
												|| link.contains("Cape") || link.contains("Ammu")
												|| link.contains("Hand")) && idx == Equipment.RANGED_STRENGTH) {

...
public final class SendPlayerUpdate extends OutgoingPacket {
	...
	private static void addNewPlayer(PacketBuilder packetBuf, Player player, Player other) {
		packetBuf.writeBits(11, other.getIndex()).writeBit(true).writeBit(true)
				.writeBits(5, other.getY() - player.getY()).writeBits(5, other.getX() - player.getX());
	}
	...
	private static void updatePlayer(PacketBuilder blockBuf, Player player, Player other, UpdateState state) {
		...
		if (other.updateFlags.contains(UpdateFlag.CHAT) && state != UpdateState.UPDATE_SELF
				&& other.getChatMessage().isPresent()) {
			mask |= 0x80;
		}
	...
	private static void appendForceMovementMask(Player player, Player other, PacketBuilder blockBuf) {
		...
		blockBuf.writeByte(startX, ByteModification.SUB).writeByte(startY, ByteModification.SUB)
				.writeByte(startX + endX, ByteModification.SUB).writeByte(startY + endY, ByteModification.SUB)
	...
	private static void appendFaceCoordinteMask(Player other, PacketBuilder blockBuf) {
		...
		if (loc == null) {
			...
			blockBuf.writeShort(((currentPos.getX() + currentDir.getDirectionX()) << 1), ByteModification.ADD,
					ByteOrder.LE).writeShort(((currentPos.getY() + currentDir.getDirectionY()) << 1) + 1, ByteOrder.LE);
		} else {
	...
	private static void appendChatMask(PacketBuilder blockBuf, Player other) {
		...
		blockBuf.writeShort(((message.getColor().getCode() & 0xFF) << 8) | (message.getEffect().getCode() & 0xFF),
				ByteOrder.LE).writeByte(other.right.getCrown()).writeByte(encoded.length, ByteModification.NEG)
	...
	private static void appendAppearanceMask(Player other, PacketBuilder blockBuf) {
		...
		tempBuf.writeByte(other.appearance.getGender().ordinal()).writeByte(other.headIcon)
				.writeByte(other.skulling.getHeadIconType().getCode()).writeByte(other.valueIcon);
		...
		tempBuf.writeByte(other.appearance.getHairColor()).writeByte(other.appearance.getTorsoColor())
				.writeByte(other.appearance.getLegsColor()).writeByte(other.appearance.getFeetColor())
				.writeByte(other.appearance.getSkinColor()).writeShort(other.mobAnimation.getStand())
				.writeShort(other.mobAnimation.getTurn()).writeShort(other.mobAnimation.getWalk())
				.writeShort(other.mobAnimation.getTurn180()).writeShort(other.mobAnimation.getTurn90CW())
				.writeShort(other.mobAnimation.getTurn90CCW()).writeShort(other.mobAnimation.getRun())
				.writeLong(Utility.hash(other.getName())).writeString(other.playerTitle.getTitle())
				.writeInt(other.playerTitle.getColor())
				.writeString(other.clanChannel == null ? "" : other.clanChannel.getOwner()).writeString(other.clanTag)
				.writeString(other.clanTagColor).writeLong(Double.doubleToLongBits(other.skills.getCombatLevel()))
				.writeByte(other.right.getCrown()).writeShort(0);
	...
	private static void updateMovement(Player player, PacketBuilder packetBuf) {
		...
		if (teleported) {
			packetBuf.writeBit(true).writeBits(2, 3).writeBits(2, player.getHeight())
					.writeBits(1, player.regionChange ? 0 : 1).writeBit(updateRequired)
			...
			packetBuf.writeBit(true).writeBits(2, 2).writeBits(3, player.movement.getWalkingDirection())
					.writeBits(3, player.movement.getRunningDirection()).writeBit(player.isUpdateRequired());
		} else if (player.movement.getWalkingDirection() != -1) {
			packetBuf.writeBit(true).writeBits(2, 1).writeBits(3, player.movement.getWalkingDirection())

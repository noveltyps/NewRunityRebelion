package io.server.game.world.items.containers.pricechecker;
...
public class PriceChecker extends ItemContainer {
	...
	private final int[] STRINGS = { 48550, 48551, 48552, 48553, 48554, 48555, 48556, 48557, 48558, 48559, 48560, 48561,
	...
	public void searchItem(String name) {
		...
		if (searched == null) {
			...
			player.send(
					new SendString(searchedItem == null ? "" : "<col=ffb000>" + searchedItem.getName() + ":", 48582));
			player.send(new SendString(
					searchedItem == null ? "" : Utility.formatDigits(searchedItem.getValue(priceType)), 48583));
		}
	}
	...
	public void onRefresh() {
		for (int index = 0; index < STRINGS.length; index++) {
			...
			if (getItems()[index] != null) {
				...
				value = getItems()[index].isStackable()
						? Utility.formatDigits(amount) + " x " + Utility.formatDigits(price) + "\\n" + "= "
								+ Utility.formatDigits(price * amount)
						: Utility.formatDigits(price);
			}
		...
		player.send(new SendString(searchedItem == null ? "" : Utility.formatDigits(searchedItem.getValue(priceType)),
				48583));
		player.send(new SendString(
				"" + (priceType == PriceType.VALUE ? NumberFormat.getInstance().format(containerValue(PriceType.VALUE))
						: NumberFormat.getInstance().format(containerValue(PriceType.HIGH_ALCH_VALUE))),
				48513));
	}

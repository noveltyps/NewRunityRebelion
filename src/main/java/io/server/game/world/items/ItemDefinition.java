package io.server.game.world.items;
...
import static io.server.game.world.entity.combat.CombatConstants.EMPTY_REQUIREMENTS;
import static io.server.game.world.entity.combat.CombatConstants.SKILL_REQUIREMENT_CONFIG_FIELD_NAMES;
...
import io.server.util.parser.JsonSaver;
...
public class ItemDefinition {
		...
		for (ItemDefinition definition : definitions) {
			...
			for (int index = 0; index < SKILL_REQUIREMENT_CONFIG_FIELD_NAMES.length; index++) {
				if (definition.requirements[index] > 0)
					saver.current().addProperty(SKILL_REQUIREMENT_CONFIG_FIELD_NAMES[index],
	...
	public static ItemDefinition get(int id) {
		if (id < 0 || DEFINITIONS == null ||  id >= DEFINITIONS.length-1) {
			return DEFAULT;
		}

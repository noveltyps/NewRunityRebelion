...
import io.server.content.activity.impl.raids2.Raids2;
...
public class Raids24Session extends DynamicRegionHandler {
	...
	public void onStart(Player player) {
		...
		for (Player p : partners)
			p.locking.lock();
		player.send(new SendFadeScreen("Welcome to the Raids 2!", 1, 3));
		World.sendMessage(player.getName() + " Was Litty enough to take on Raids 2!");
		World.schedule(5, () -> {
			Raids2.create(player, partners);
...
//		player.getRaidPartners().clear();
	...
	public void resetPartner() {
		...
		for (Player p : partners) {
			...
			p.sendMessage("Your raids 2 partners have been reset!");
		}
...
//		player.setAllForOnePartner(null);
//		player.message("You have reset your Raids 2 partner!");
//		partner.message("You have reset your Raids 2 partner!");
//		} else {
//			player.message("stfu");
//		}
	}
...
//		partner.sendMessage(message);
	...
	public static void sendPartnerRequest(Player player, Player other) {
		...
		f.sendOption("Accept "+player.getUsername()+"'s Raids 2 partner request", () -> f.onAction(() -> {
			f.clear();
...
//			player.sendPartnerMessage(player, other, "You have been partnered! Make your way over to the Raids 2 Portal!");

...
import io.server.content.activity.impl.raids3.Raids3;
...
public class Raids34Session extends DynamicRegionHandler {
	...
	public void onStart(Player player) {
		...
		for (Player p : partners)
			p.locking.lock();
		player.send(new SendFadeScreen("Welcome to the Raids 3!", 1, 3));
		World.sendMessage(player.getName() + " Was Litty enough to take on Raids 3!");
		World.schedule(5, () -> {
			Raids3.create(player, partners);
...
//		player.getRaidPartners().clear();
	...
	public void resetPartner() {
		...
		player.getRaidPartners().clear();
		player.message("You have reset your Raids 3 partner!");
		
		for (Player p : partners) {
			p.getRaidPartners().clear();
			p.sendMessage("Your Raids 3 partners have been reset!");
		}
...
//		player.setAllForOnePartner(null);
//		player.message("You have reset your Raids 3 partner!");
//		partner.message("You have reset your Raids 3 partner!");
//		} else {
//			player.message("stfu");
//		}
	}
...
//		partner.sendMessage(message);
	...
	public static void sendPartnerRequest(Player player, Player other) {
		...
		f.sendOption("Accept "+player.getUsername()+"'s Raids 3 partner request", () -> f.onAction(() -> {
			f.clear();
...
//			player.sendPartnerMessage(player, other, "You have been partnered! Make your way over to the Raids 3 Portal!");

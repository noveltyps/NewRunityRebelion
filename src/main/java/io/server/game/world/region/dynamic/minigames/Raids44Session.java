...
public class Raids44Session extends DynamicRegionHandler {
	...
	public void onStart(Player player) {
		...
		for (Player p : partners)
			p.locking.lock();
		player.send(new SendFadeScreen("Welcome to the Raids 4!", 1, 3));
		World.sendMessage(player.getName() + " Was Litty enough to take on Raids 4!");
		World.schedule(5, () -> {
			Raids4.create(player, partners);
...
//		player.getRaidPartners().clear();
	...
	public void resetPartner() {
		...
		player.getRaidPartners().clear();
		player.message("You have reset your Raids 4 partner!");
		
		for (Player p : partners) {
			p.getRaidPartners().clear();
			p.sendMessage("Your Raids 4 partners have been reset!");
		}
...
//		player.setAllForOnePartner(null);
//		player.message("You have reset your Raids 4 partner!");
//		partner.message("You have reset your Raids 4 partner!");
//		} else {
//			player.message("stfu");
//		}
	}
...
//		partner.sendMessage(message);
	...
	public static void sendPartnerRequest(Player player, Player other) {
		...
		f.sendOption("Accept "+player.getUsername()+"'s Raids 4 partner request", () -> f.onAction(() -> {
			f.clear();
...
//			player.sendPartnerMessage(player, other, "You have been partnered! Make your way over to the Raids 4 Portal!");

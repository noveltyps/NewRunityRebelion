...
import io.server.content.activity.impl.arcademinigame.ArcadeMinigame;
...
public class ArcadeMinigame4Session extends DynamicRegionHandler {
	...
	public void onStart(Player player) {
		...
		player.locking.lock();
		player.send(new SendFadeScreen("Welcome to the Arcade Minigame!", 1, 3));
		World.sendMessage(player.getName() + " Was Litty enough to take on Arcade Minigame!");
		World.schedule(5, () -> {
			ArcadeMinigame.create(player, partner);
	...
	public void resetPartner() {
		...
		partner.setAllForOnePartner(null);
		player.setAllForOnePartner(null);
		player.message("You have reset your Arcade Minigame partner!");
		partner.message("You have reset your Arcade Minigame partner!");
		} else {
			player.message("stfu");
		}
	}
	...
	public static void sendPartnerRequest(Player player, Player other) {
		...
		f.sendOption("Accept "+player.getUsername()+"'s Arcade Minigame partner request", () -> f.onAction(() -> {
			f.clear();
			player.setAllForOnePartner(player, other);
			player.sendPartnerMessage(player, other, "You have been partnered! make your way over to the Arcade Minigame Portal!");

...
public enum RangedEffects {
	OPAL_BOLTS(new CombatImpact() {
		...
		final ImmutableSet<Integer> affectedNpcs = ImmutableSet.of(941, 55, 54, 53, 50, 5362, 1590, 1591, 1592, 5363,
				110, 1633, 1634, 1635, 1636, 1019, 2591, 2592, 2593, 2594, 2595, 2596, 2597, 2598, 2599, 2600, 2601,
				2602, 2603, 2604, 2605, 2606, 2607, 2608, 2609, 2610, 2611, 2612, 2613, 2614, 2615, 2616, 2627, 2628,
				2629, 2630, 2631, 2631, 2734, 2735, 2736, 2737, 2738, 2739, 2740, 2741, 2742, 2743, 2744, 2745, 2746);
		...
		@Override
		public boolean canAffect(Mob attacker, Mob defender, Hit hit) {
			if (defender.isPlayer() && defender.getPlayer().equipment.containsAny(1540, 11283, 13686)) {
				return false;
			}
	...
	GUAM_TAR(new CombatImpact() {
		...
		public void impact(Mob attacker, Mob defender, Hit hit, List<Hit> hits) {
			...
			CombatType type = fightType.equals(FightType.FLARE) ? CombatType.RANGED
					: fightType.equals(FightType.SCORCH) ? CombatType.MELEE : CombatType.MAGIC;

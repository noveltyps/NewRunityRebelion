 ...
 */
public final class SecretsChestAction4 extends Action<Player> {

	//private final GameObject object;

	public SecretsChestAction4(Player player/*, GameObject object*/) {
		super(player, 1);
		//this.object = object;
	}
	...
	protected void onSchedule() {
		getMob().locking.lock();
		getMob().inventory.remove(3467, 1);
	...
	protected void onCancel(boolean logout) {
		...
		getMob().inventory.add(SecretsChest4.getReward());
		AchievementHandler.activate(getMob(), AchievementKey.SECRETS_CHEST_4, 1);
		getMob().send(new SendMessage("...you find a few items inside of the chest."));
		getMob().activityLogger.add(ActivityLog.SECRETS_CHEST_4);
	}
